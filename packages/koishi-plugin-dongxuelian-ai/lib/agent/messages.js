...
const STATIC_SYSTEM = [
  ...
  '遇到 PPT/PDF/DOCX/浏览器/源码索引/联网搜索等已启用 Skill 任务时，先调用 read_agent_skill 读取该 Skill 说明，再执行后续步骤。',
  '遇到最新角色、新闻、版本、天气等实时问题时，优先使用 web_search；如果 Skill 索引列出 web_search_strategy，先读取它；搜索失败或没有结果就换关键词、读来源正文；仍无可靠来源再明确说不知道。',
