...
const THINKING_OUTPUT_RE = /根据系统指令|根据规则|根据系统约束|作为东雪莲|在群聊中(?:主动)?插话|我的角色是|当前场景|规则[：:]|可能太|这是一个.{0,8}(?:模式|回复|场景)|需要.{0,10}(?:回复|插话|吐槽)|可以吐槽|比较随意/
...
function setGroupPersona(channelKey, personaName, hostileCapable) {
  ...
  require('fs').writeFileSync(PERSONA_GROUPS_FILE, JSON.stringify(personaGroupsCache, null, 2), 'utf8')
}
...
function getAvailablePersonas() {
  ...
  try {
    ...
    for (const entry of entries) {
      if (!entry.isFile() || !/^SKILL(\.[\w-]+)?\.md$/i.test(entry.name)) continue
...
async function loadSkills() {
  ...
  // 只加载 core + modes，personas 按群动态加载
  await walk(SKILLS_CORE_DIR)
  await walk(SKILLS_MODES_DIR)
  skillsCache = skills
  return skills
}
...
async function chat(session, userText, ctx, options = {}) {
  ...
  // 人格系统：检查当前群是否有自定义人格
  const channelKey = getChannelKey(session)
  const groupPersonaEntry = getGroupPersona(channelKey)
  const personaName = groupPersonaEntry?.persona || null
  let personaHostileCapable = groupPersonaEntry?.hostile_capable
  let personaSkillContent = null
  if (personaName) {
    personaSkillContent = loadPersonaSkill(personaName)
    // 读取 persona 的 YAML 默认值（如果群配置没有覆盖）
  ...
  if (testMode) {
    ...
    // 有自定义人格 + 人格具备嘴臭能力 → 用人格自己的被惹毛规则
    systemPrompt = buildFriendlySystemPrompt()
    if (personaSkillContent) systemPrompt += '\n\n---\n\n' + personaSkillContent
  } else if (hostile) {
    // 无自定义人格，或人格不具备嘴臭能力 → 标准嘴臭
    systemPrompt = buildAbusiveSystemPrompt()
  } else {
    // 正常模式
    systemPrompt = buildFriendlySystemPrompt()
    if (personaName && personaSkillContent) systemPrompt += '\n\n---\n\n' + personaSkillContent
  }

  ctx.logger('dongxuelian-ai').debug(`mode=${hostile ? 'abusive' : 'friendly'} persona=${personaName || 'none'} hostile_capable=${personaHostileCapable} input=${userText.slice(0, 60)}`)
...
ctx.logger('dongxuelian-ai').info(`middleware-debug: plain=${JSON.stringify(plain).slice(0, 100)} directAt=${directAt} isDirect=${!!session.isDirect}`)
    ...
    // 人格系统指令
    if (/^东雪莲人格$/.test(plain)) {
      if (!hasAdminPermission(session)) return '只有管理员才能查看人格状态。'
      const entry = getGroupPersona(channelKey)
      if (!entry) return `当前群：默认模式（自动友善/嘴臭切换）\n嘴臭能力：由系统自动判断`
      const meta = parsePersonaFrontmatter(loadPersonaSkill(entry.persona) || '')
      ...
      return `当前群人格：${entry.persona}\n嘴臭能力：${hostileStatus}`
    }

    if (/^东雪莲人格列表$/.test(plain)) {
      if (!hasAdminPermission(session)) return '只有管理员才能查看人格列表。'
      const personas = getAvailablePersonas()
      if (personas.length === 0) return '当前没有人格配置。'
      const current = getGroupPersona(channelKey)
      const lines = personas.map(p => {
        const marker = current?.persona === p.name ? ' ← 当前' : ''
        return `- ${p.name}（${p.description || '无描述'}）${marker}`
      })
      return `可用人格：\n${lines.join('\n')}`
    }

    const personaSwitchMatch = plain.match(/^东雪莲人格切换\s+(.+)$/)
    if (personaSwitchMatch) {
      if (!hasAdminPermission(session)) return '只有管理员才能切换人格。'
      if (!inGuild) return '人格切换只能在群里用。'
      const targetName = personaSwitchMatch[1].trim()
      const personas = getAvailablePersonas()
      const found = personas.find(p => p.name === targetName)
      if (!found) return `未找到人格"${targetName}"。可用：${personas.map(p => p.name).join('、')}`
      setGroupPersona(channelKey, targetName, null)
      return `已切换到人格：${targetName}`
    }

    if (/^东雪莲人格重置$/.test(plain)) {
      if (!hasAdminPermission(session)) return '只有管理员才能重置人格。'
      if (!inGuild) return '人格重置只能在群里用。'
      resetGroupPersona(channelKey)
      return '已重置为默认模式（自动友善/嘴臭切换）。'
    }

    const hostileMatch = plain.match(/^东雪莲嘴臭\s*(开|关)$/)
    if (hostileMatch) {
      if (!hasAdminPermission(session)) return '只有管理员才能设置嘴臭能力。'
      if (!inGuild) return '嘴臭设置只能在群里用。'
      const entry = getGroupPersona(channelKey)
      if (!entry) return '当前无人格设置，嘴臭由系统自动判断，无需手动开关。'
      const enable = hostileMatch[1] === '开'
